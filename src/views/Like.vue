<template>
	<div class="like">
		<div class="navbar">
			<van-nav-bar title="我的收藏" right-text="全部清除" :fixed='true' left-arrow @click-left="back" @click-right="allclear" />
		</div>
	...
	export default {
		...
		methods: {
			...
			initial(){
				...
				let dataLike = JSON.parse(localStorage.getItem("likeSong"));
			...
			//清除全部
			allclear() {
				localStorage.setItem('likeSong', JSON.stringify([]));
				this.$store.commit('LikeModule/emptyData');
			},
			...
			removeData(pid,index){
				...
				for(let i = 0 ; i<dataLike.length ; i++){
					if(dataLike[i].id == pid){
						dataLike.splice(i,1);
						localStorage.setItem('likeSong', JSON.stringify(dataLike));
						this.$store.commit('LikeModule/emptyData');
						this.$store.commit('LikeModule/changeData', {
							key: 'allLikeData',
							value: dataLike
